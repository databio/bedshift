...
class Bedshift(object):
    ...
    def _find_intersection(self, fp, threshold=1):
        ...
        comparare_bed = pybedtools.example_bedtool(os.path.abspath(fp))

        intersect_ref = reference_bed.intersect(comparare_bed, u=True).to_dataframe()
    ...
    def all_perturbations(self, bedfile=None,
...
def handle_yaml(bedshifter, yaml_fp):
    ...
    for operation in operations:
        ...
        if set(['add_from_file', 'file', 'rate']) == set(list(operation.keys())):
            fp = operation['file']
        ...
        ##### shift reference bedfile with configs #####
        ...
        ##### merge reference bedfile #####
            ...
            print("Invalid settings entered in the config file. Please fix and try again!")
            sys.exit(1)
